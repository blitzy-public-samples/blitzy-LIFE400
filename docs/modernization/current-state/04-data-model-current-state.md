...
`USERID`, `REQUESTING USER ID` [QDDSSRC/SVCPF.pf:L51], is declared and never populated. The reason is in the only program that writes the file: `SVCMNT` describes the record as a single undifferentiated two-hundred-byte area, `01 SVCPF-RECORD PIC X(200).` [QCBLLESRC/SVCMNT.cbl:L58], and writes that area whole [QCBLLESRC/SVCMNT.cbl:L191]. The name `SVCPF-RECORD` occurs exactly twice in the entire estate — that declaration and that write — so no statement anywhere moves a value into it before it is written. A program that writes an undifferentiated area cannot populate an individual column, and the column therefore has no writer. This document records the fact; the disposition belongs to [the known defects and stubs register](07-known-defects-and-stubs.md), and the structural analysis of why both secondary files are declared by the programs that never touch them is already given in [the current-state architecture](02-architecture-current-state.md).

## The claims file

`CLMPF` records one claim per record [QDDSSRC/CLMPF.pf:L10]. It is the widest of the three files in column count and the one that carries the most sensitive content.
...
### Categories of sensitive data in the claim record
...
`QCPYSRC/POLDATA.cpy` is 175 lines defining a single record, `01 WS-POLICY-MASTER-REC.` [QCPYSRC/POLDATA.cpy:L14]. It is a [copybook](../reference/glossary-ibm-i.md#copybook) — a source fragment expanded into each consumer at compile time — and it is simultaneously the record description of the policy master in every program that uses it, because in all seven consumers the `COPY` statement sits immediately after `FD POLMST` [QCBLLESRC/NBUWMNT.cbl:L49-L50]. It is therefore both the in-memory shape of a policy and the declared shape of the stored record, and this document is the first to inventory it.

### Group structure of the contract

Nine level-05 groups partition the record. The line ranges below run from each group's own declaration to the line before the next group's, so together they cover the whole record with no gap.
...
The item counts sum to 9 + 13 + 10 + 13 + 8 + 6 + 10 + 18 + 2 = **89**.
...
- **84 level-10 items carrying a `PIC` clause.** These are the ordinary fields of the record.
- **5 level-15 items**, all inside the rider table, at [QCPYSRC/POLDATA.cpy:L90-L94].
- 84 + 5 = **89 elementary items.**
...
The contract's declared length can be computed from its own `PIC` clauses. Summing them group by group — a `V` marking an implied decimal point occupies no byte, an embedded sign occupies none, and the rider table contributes its 42-byte occurrence five times — gives 146 + 77 + 66 + 267 + 120 + 48 + 56 + 181 + 18 = **979 bytes**.
...
Two consequences follow, and both are load-bearing for everything downstream of this document. The correspondence between a contract item and a column can only be established by name, declared length and declared scale — never by byte offset — which is exactly the method used for the classification below. And the contract is not a description of one file: it carries the servicing and claim sections of the business alongside the policy sections, in one record area, which is why a single `REWRITE` of the policy master is the only write most of this data ever sees.
...
- **Exactly one item in the whole contract is declared signed.** It is `PM-PREMIUM-DELTA PIC S9(13)V99.` [QCPYSRC/POLDATA.cpy:L106]. Every other numeric item among the 89 is unsigned, so a negative intermediate value has nowhere to live in the rest of the record. The persisted column that corresponds to it, `PREMDLT` [QDDSSRC/SVCPF.pf:L45], is declared exactly like every other monetary column, `15S 2`, and the DDS field line carries no separate sign attribute — so the only place in the estate where a sign is stated explicitly is that one contract item.

### The outcome pair every feature reports through

Two items in the control area are the system's entire outcome channel: `PM-RETURN-CODE PIC 9(02) VALUE 0.` [QCPYSRC/POLDATA.cpy:L36] and `PM-RETURN-MESSAGE PIC X(100) VALUE SPACES.` [QCPYSRC/POLDATA.cpy:L37]. They are data-model items rather than parameters — they sit inside the record area, not in linkage storage — and neither has a column in any file, so an outcome is communicated by being placed in the record being processed and is overwritten by the next operation on that record. They are noted here because they are two of the 89 items and two of the transient ones; how they function as an integration mechanism is analysed in [the current-state architecture](02-architecture-current-state.md).

## Coded domains: the level-88 condition names

A [level-88 condition name](../reference/glossary-ibm-i.md#level-88-condition-name) gives a name to one literal value of the field above it. It occupies no storage and constrains nothing: it is a named comparison. The contract declares **48** of them across 14 coded fields, and they are the only place in LIFE400 where the legal values of a coded field are written down as code rather than as descriptive text. Each of the 48 is mapped to a target constraint in [the target data model and schema mapping](../target-state/03-target-data-model-and-schema-mapping.md).
...
The band sizes sum to 8 + 3 + 2 + 2 + 4 + 4 + 2 + 6 + 3 + 1 + 5 + 2 + 3 + 3 = **48**.
...
Of the 89 elementary items the contract declares, **57 correspond to a persisted column and 32 do not.** An item is classified as persisted when a column exists whose name, declared length and declared scale correspond to it; the method is correspondence rather than byte offset, for the reason recorded under [the contract is larger than the file it describes](#the-contract-is-larger-than-the-file-it-describes).
...
57 + 5 + 2 = **64**, which closes the column arithmetic from both sides.

One further fact belongs with this, because it bounds what can be said about 36 of the 64 columns. Neither secondary file's record area is ever populated: `SVCPF-RECORD` appears twice in the estate, as a declaration [QCBLLESRC/SVCMNT.cbl:L58] and a write [QCBLLESRC/SVCMNT.cbl:L191], and `CLMPF-RECORD` likewise [QCBLLESRC/CLMMNT.cbl:L57], [QCBLLESRC/CLMMNT.cbl:L179]; the batch programs' structured declarations of the same two files [QCBLLESRC/SVCBILB.cbl:L66-L69], [QCBLLESRC/CLMADJB.cbl:L60-L63] take part in no I/O at all. So the 15 servicing and 21 claim columns receive no field-level move from any program, while the values they name are held in the contract's own servicing and claim groups [QCPYSRC/POLDATA.cpy:L117-L136], [QCPYSRC/POLDATA.cpy:L138-L170] and travel with the policy-master rewrite [QCBLLESRC/SVCMNT.cbl:L190], [QCBLLESRC/CLMMNT.cbl:L178]. Any migration must therefore treat the two secondary files as schemas whose population cannot be verified from source, which is a reconciliation problem for [the data migration runbook](../migration/04-data-migration-runbook.md).

### Contract items no program references

Four of the 89 items are declared, correspond to a column, and are referenced by no program anywhere in the estate:
...
Each of the four has a column waiting for it and no code that ever reads or writes the item. One of them matters more than the other three: `PM-LAST-MAINT-DATE` [QCPYSRC/POLDATA.cpy:L114] corresponds to a *policy-master* column [QDDSSRC/POLMST.pf:L75], the file whose columns are otherwise reached by name from program code — so `LSTMNT` is the one policy-master column in the schema with no writer at all. The other three correspond to claim columns which, as recorded immediately above, receive no field-level move from any program in any case. The observation is recorded here as a coverage fact about the schema; the disposition belongs to [the known defects and stubs register](07-known-defects-and-stubs.md).

## Constraints the schema does not declare

Each absence below was established by searching all ten members of `QDDSSRC`, not only the four database members, so a keyword used anywhere in the estate's DDS would have been found. They are stated as structural facts; what each one costs and what would close it belongs to the risk and target-state documents named against them.
...
That combines with a second absence to fix the retention position structurally. A census of the code lines of all eight COBOL members finds **zero** `DELETE` operations, so no program in LIFE400 can remove a record from any file. Records accumulate for as long as the files exist, and there is no erasure path of any kind. This document states the structural fact and stops; retention exposure, minimisation and erasure are analysed in [the compliance and data protection document](../risk/02-compliance-and-data-protection.md).

### A record key that does not correspond to its file's key
...
So one side names a composite area beginning with an amendment-type code and the other names a single service-request identifier. Two adjacent facts are recorded alongside, because they bound what can be concluded. Every other record-key declaration in the estate *does* correspond to its file's key: seven programs key the policy master on `PM-POLICY-ID` [QCBLLESRC/NBUWMNT.cbl:L42], [QCBLLESRC/NBUWB.cbl:L54], [QCBLLESRC/SVCMNT.cbl:L42], [QCBLLESRC/SVCBILB.cbl:L52], [QCBLLESRC/CLMMNT.cbl:L41], [QCBLLESRC/CLMADJB.cbl:L46], [QCBLLESRC/POLMSTINQ.cbl:L42] against `K POLID` [QDDSSRC/POLMST.pf:L81], and two key the claims file on `PM-CLAIM-ID` [QCBLLESRC/CLMMNT.cbl:L47], [QCBLLESRC/CLMADJB.cbl:L52] against `K CLMID` [QDDSSRC/CLMPF.pf:L67]. And the contract contains no elementary item corresponding to `SVCID` at all, as recorded under [columns the contract does not describe](#columns-the-contract-does-not-describe) — so there was no single item available to name.

This document records the disagreement and does not resolve it, because resolving it changes what "the key of this file" means and that is a decision with consequences for the mapping. The disposition belongs to [the known defects and stubs register](07-known-defects-and-stubs.md) and the mapping to [the target data model and schema mapping](../target-state/03-target-data-model-and-schema-mapping.md).
...
- **Any change to the contract obliges recompilation of all seven consumers.** Adding a column to a file means adding an item to the contract, and adding an item to the contract means rebuilding every program that copies it — including the read-only inquiry program, which has no interest in the change.
- **There is no compatibility boundary.** A program compiled against one expansion and a program compiled against a later one are indistinguishable at run time: nothing carries a schema identifier, nothing is negotiated and nothing is checked. Agreement between two programs about the shape of a policy is a build-time property, maintained by rebuilding them together.
- **The persisted layout and the in-memory layout are one declaration, not two kept in step.** Because the expansion lands in the file section rather than in working storage, a program cannot hold a policy in memory in any shape other than the shape it declares for the file.
- **The contract, not the schema, is the authoritative statement of the domain.** Since 33 of the 48 domains are tested nowhere and the DDS declares no constraint at all, the only written statement of what most coded fields may hold is this one 175-line member.
...
erDiagram
    POLMST["QDDSSRC/POLMST.pf — policy master, format POLMSTREC L14, 28 columns, 233 bytes"] {
        alpha POLID PK "12A L16 — key K POLID L81, not UNIQUE"
        alpha APPID "12A L18"
    ...
    SVCPF["QDDSSRC/SVCPF.pf — service amendments, format SVCREC L14, 15 columns, 127 bytes"] {
        alpha SVCID PK "12A L16 — key K SVCID L54, not UNIQUE, no contract item"
        alpha POLID FK "12A L18 — undeclared reference"
        ...
        zoned PREMDLT "15S 2 L45 — contract item is the only signed one"
    ...
    CLMPF["QDDSSRC/CLMPF.pf — claims, format CLMREC L14, 21 columns, 202 bytes"] {
        alpha CLMID PK "12A L16 — key K CLMID L67, not UNIQUE"
        alpha POLID FK "12A L18 — undeclared reference"
    ...
    POLMST ||..|| POLMSTL1 : "access path on the same key POLID, adds no ordering — QDDSSRC/POLMSTL1.lf:L14"
    POLMST ||..o{ SVCPF : "POLID to POLID — UNDECLARED, no referential integrity in DDS"
    POLMST ||..o{ CLMPF : "POLID to POLID — UNDECLARED, no referential integrity in DDS"
```
...
The transitions below are the complete set. They come from a census of every statement in the eight COBOL members that writes the field: there are **thirteen**, and no program uses the `SET` form of a condition name to change it, so the census is exhaustive.
...
| `GR` | [QCBLLESRC/SVCBILB.cbl:L204], [QCBLLESRC/SVCMNT.cbl:L172] | Status is active and days since paid is greater than zero and no greater than the plan's grace days [QCBLLESRC/SVCBILB.cbl:L201-L203], [QCBLLESRC/SVCMNT.cbl:L170-L171] |
| `LA` | [QCBLLESRC/SVCBILB.cbl:L209], [QCBLLESRC/SVCMNT.cbl:L176] | Status is active or grace, and days since paid exceeds the plan's grace days [QCBLLESRC/SVCBILB.cbl:L207-L208], [QCBLLESRC/SVCMNT.cbl:L174-L175] |
| `RS` | [QCBLLESRC/SVCBILB.cbl:L414], [QCBLLESRC/SVCMNT.cbl:L334] | Status is lapsed [QCBLLESRC/SVCBILB.cbl:L395], [QCBLLESRC/SVCMNT.cbl:L317] and the elapsed period is within the plan's reinstatement window [QCBLLESRC/SVCBILB.cbl:L404] |
...
- **Every transition is a `MOVE` into a record area, not a checked operation.** The field is a bare two-character column [QDDSSRC/POLMST.pf:L25] with no declared domain, so nothing in the schema prevents a value outside these eight, and nothing records that a transition occurred: the previous status is overwritten and the only history is the audit pair, which holds a program name and a date [QCPYSRC/POLDATA.cpy:L173-L175].

## Figures owned by other documents

This document owns the column inventories, the record lengths, the contract's group and item counts, the level-88 domain inventory and its enforcement census, the persisted-versus-transient classification and the contract-status transition census. Every other quantity in the assessment has exactly one owning document, and restating one here would create a second place for it to be wrong.
...
- DDS display members, cited for the rider fields and the menu date field — `QDDSSRC/NBUWDSPF.dspf`, `QDDSSRC/MNUDSPF.dspf`
