...
| Adopted authority | Program runs with its owner's authority, not the caller's | Service account or assumed role | Not instantiated: every one of the thirteen `CRTPGM` commands creates its program with no user-profile parameter [README.md:L169-L176], [README.md:L188-L192], so each runs under the caller's authority |
...
| Job description | Named bundle of job attributes, supplying only what a submission leaves unstated | Job template or worker configuration profile | [README.md:L199] |
...
| Override | Redirection of a file reference, confined to the job that issued it | Scoped configuration binding or DI override | [QCLSRC/DLYUPD.clle:L60] |
...
- **In LIFE400** — every interactive path runs over it. The README's screen reference lists the formats a user actually sees [README.md:L222], and the repository structure identifies the menu display file as a 5250 device [README.md:L35]. **No transport security is declared for it anywhere in this repository**, and the statement is deliberately that narrow: whether a given installation front-ends the datastream with a secured emulator or tunnels it is a platform and network configuration matter, and this repository contains no subsystem description, no device configuration and no network definition of any kind from which either answer could be read. What the estate does establish is that the application itself contributes nothing: the census of all eight COBOL members published by [the current-state architecture](../current-state/02-architecture-current-state.md) finds no identity, session or encryption logic anywhere, and in the one place a signed-on identity is available it is moved straight to a display field and dropped [QCBLLESRC/MAINMENU.cbl:L57-L58]. That exposure is not assessed here: it is registered with severity and evidence in [the security risk register](../risk/01-security-risk-register.md), and the control that closes it is designed in [the target security control design](../target-state/04-security-control-design.md).
...
- **Modern equivalent** — a shell script or a small orchestration program, with three differences that matter. A CL program is compiled to a program object like any other and is called by name; its parameters are fixed-length typed values passed positionally, so a declared type and length govern each argument but nothing binds a caller's argument list to the callee's declaration at compile time — the match is by position and by convention, and a mismatch surfaces at run time; and error handling is a language feature rather than a convention, which is why the CPF message entry below is inseparable from this one.
...
- **In LIFE400** — one job description, created for the application and pointing at the application job queue [README.md:L199], and named by each batch submitter at the moment of submission [QCLSRC/RUNNBUW.clle:L43]. What it contributes is whatever a submission does not state for itself, and in this estate the submissions state a great deal.
- **Two attributes it does not govern here**, both easy to assume it does. **The queue:** every submission names the job queue explicitly, on the same command that names the job description [QCLSRC/RUNNBUW.clle:L43], [QCLSRC/RUNSVC.clle:L50], [QCLSRC/RUNCLM.clle:L50], and a queue named on the command is the queue the job goes to — so editing the job description's queue would not move this application's batch work. **The library list:** the creation command supplies a queue and a text description and no initial library list at all [README.md:L199], and no submission names an initial library list or a current library either [QCLSRC/RUNNBUW.clle:L43-L46], [QCLSRC/RUNSVC.clle:L50-L53], [QCLSRC/RUNCLM.clle:L50-L53] — and the command defaults for both are the *submitting* job's, so the submitted job inherits the caller's name resolution rather than anything the job description holds. Why that inheritance, and not the file overrides, decides which files a batch program actually opens is set out in [the operational model](../current-state/06-operational-model.md).
- **Modern equivalent** — a job template, a worker configuration profile, or a container job specification: a reusable bundle of execution settings referenced by name. The property to carry forward carefully is the one above: a template only supplies what the call site leaves unset, so reading the template alone never tells you how a job ran.
...
- **In LIFE400** — one job queue serves the whole application [README.md:L200], and all three batch submitters name it explicitly [QCLSRC/RUNNBUW.clle:L43], [QCLSRC/RUNSVC.clle:L50], [QCLSRC/RUNCLM.clle:L50], which is what settles the queue at the call site rather than in the job description they also name.
...
- **What it is** — a command that redirects a file reference without changing the program. A program compiled against an unqualified file name can be pointed at a specific library's copy of that file at run time. The redirection is scoped, and the scope bounds it in two independent ways that this estate depends on.
- **How far it reaches, and this is the property the estate turns on.** A job-scoped override applies to every program that runs *in the job that issued it*, however deeply called synchronously. It is not part of the environment [`SBMJOB`](#sbmjob) hands to the job it creates: a submitted job begins with its own overrides and none of the submitter's, while the submitter's remain in force in the submitter. The submitted program therefore resolves its unqualified file names from its own [library list](#library-list) — which, because no submission names one, is the list the submitting job happened to have rather than anything the [job description](#job-description) supplies. Where each mechanism actually governs which file a LIFE400 program opens is established by [the operational model](../current-state/06-operational-model.md).
- **How long it lasts.** It ends in one of three ways — it is explicitly deleted, another override for the same file at the same scope replaces it, or the scope itself ends, which for a job-scoped override means the job ending. Deleting it is the early ending rather than the only one, so an override left in place is a hazard for the remainder of that job.
- **In LIFE400** — the nightly driver is the one place where an override does its job, because it calls the servicing engine synchronously in its own job [QCLSRC/DLYUPD.clle:L75] after redirecting both files it needs to the application library at job scope [QCLSRC/DLYUPD.clle:L60], [QCLSRC/DLYUPD.clle:L61]. It deletes both at the end [QCLSRC/DLYUPD.clle:L98-L99] with one monitored condition after the second delete [QCLSRC/DLYUPD.clle:L100]. Each batch submitter wraps the same set-up and tear-down pair around its own **submission** [QCLSRC/RUNSVC.clle:L44-L45], [QCLSRC/RUNSVC.clle:L60-L61] — so those overrides are established in the submitter's job and the program they name runs in the job the submission creates [QCLSRC/RUNSVC.clle:L50-L53], never seeing them. Six overrides across the three submitters are in that position, and their disposition is recorded in [the known defects and stubs register](../current-state/07-known-defects-and-stubs.md). The set-up statement reads:

```text
OVRDBF FILE(POLMST) TOFILE(LIFE400/POLMST) OVRSCOPE(*JOB)
```

- **Modern equivalent** — a scoped configuration binding, or a dependency-injection override registered for the lifetime of a request or a worker. The scoping caveat carries over exactly: a binding registered in one process is not inherited by work handed to another.
- **Deleting an override is itself guarded, and the guard is narrower than it looks.** `DLTOVR` raises `CPF9841` when there is no override to delete, so the deletion is written with a monitored condition. In all four CL members that delete overrides the monitor is a **command-level** `MONMSG` placed after the *second* of two `DLTOVR` commands — [QCLSRC/RUNNBUW.clle:L53-L55], [QCLSRC/RUNSVC.clle:L60-L62], [QCLSRC/RUNCLM.clle:L60-L62], [QCLSRC/DLYUPD.clle:L98-L100] — and a command-level monitor covers only the command it immediately follows. The first delete of each pair is therefore unmonitored: if that override is absent, `CPF9841` goes unhandled and clean-up is interrupted before the second delete runs. The tolerance the pattern reads as offering exists for one of the two deletions.
- The point for a modern reader: **this is configuration expressed as an imperative command.** It is established by executing a statement and withdrawn by executing another. There is no declarative file to inspect, nothing fails at build time if a set-up and tear-down pair do not match, and the effective binding of a file can only be known by tracing which statements have already run **in that job** — which first requires knowing whether the code that opens the file runs in that job at all.
...
- **"A different job" is a boundary, not a detail.** The submitted job's attributes come from what the submission names — chiefly the job description, which supplies its initial [library list](#library-list) — and not from the state of the submitting job. Job-scoped [overrides](#override) established before the submission belong to the submitter and do not cross the boundary; the submitted program resolves its unqualified file names in its own job. Nor does the submitted job start merely because it was queued: it starts when a subsystem takes it off the [job queue](#job-queue) the submission named. Everything the submitted program needs in order to open the right file is therefore a property of the job description and the queue's servicing subsystem, and [the operational model](../current-state/06-operational-model.md) records what LIFE400 does and does not supply on that path.
...
- **What the submitted job takes from elsewhere.** Whatever the command does not name, the job description supplies — with two exceptions that decide how unqualified names resolve. The initial library list and the current library are not named on any of these submissions [QCLSRC/RUNNBUW.clle:L43-L46], [QCLSRC/RUNSVC.clle:L50-L53], [QCLSRC/RUNCLM.clle:L50-L53], and the command's defaults for both are the submitting job's rather than the job description's, so the new job inherits the caller's name resolution. Overrides in force in the submitter are not inherited at all. Both facts are load-bearing for what a batch program opens, and are worked through in [the operational model](../current-state/06-operational-model.md).
- **Modern equivalent** — enqueuing a message or a task for a worker, or a scheduler API call. One property to carry forward deliberately: the submitting program holds no handle on the result. It notifies the requester that work was submitted and returns [QCLSRC/RUNNBUW.clle:L49-L50], and nothing in the design correlates a submission with its eventual outcome. A second is the inheritance above — a modern worker that silently adopts its caller's resolution context is the same defect in a new language.
...
- **In LIFE400** — money is declared twice, once on each side of the same field, in two different notations that mean the same thing. DDS declares the sum assured as fifteen digits with two decimals [QDDSSRC/POLMST.pf:L52]; the contract declares the matching item of that same name as thirteen digits plus two decimals [QCPYSRC/POLDATA.cpy:L76] — the same fifteen-digit, two-decimal precision. Every other monetary item in the contract is declared the same way, the premium results among them [QCPYSRC/POLDATA.cpy:L99]:
...
- Unsigned is the norm in the contract's own picture clauses: exactly one item in the shared record carries a leading `S` [QCPYSRC/POLDATA.cpy:L106] and every other numeric item does not, so a negative intermediate value has nowhere to live in most of the record. The stored side does not draw that distinction — every numeric column in every DDS member is the `S` type, which is signed zoned decimal, so the sign position exists in the stored field whether or not the contract item mapped to it declares one. The scale is not uniformly two either — the flat extra rate is declared with four decimals [QDDSSRC/POLMST.pf:L49] — so no single target numeric type can be chosen once and applied everywhere. Zoned is also the only numeric representation used: an exhaustive search of the DDS members finds no packed, binary, or floating-point field type anywhere.
...
The only quantities stated above are the small ones an entry needs in order to define its own construct, each cited at the point it is made — three physical files [QDDSSRC/POLMST.pf:L14], [QDDSSRC/SVCPF.pf:L14], [QDDSSRC/CLMPF.pf:L14], four display files [QDDSSRC/MNUDSPF.dspf:L19], [QDDSSRC/NBUWDSPF.dspf:L24], [QDDSSRC/SVCDSPF.dspf:L23], [QDDSSRC/CLMDSPF.dspf:L21], two printer files [QDDSSRC/POLRPT.prtf:L15], [QDDSSRC/CLMRPT.prtf:L15], one copybook [QCPYSRC/POLDATA.cpy:L14], five CL members [QCLSRC/STRTLIFE.clle:L18], [QCLSRC/RUNNBUW.clle:L21], [QCLSRC/RUNSVC.clle:L21], [QCLSRC/RUNCLM.clle:L21], [QCLSRC/DLYUPD.clle:L24], a 24-row by 80-column screen [QDDSSRC/MNUDSPF.dspf:L12]. Every *inventory* figure has exactly one owning document, and restating one here would create a second place for it to be wrong. Look those up at the owner instead:
