...
**No temporal content, and no cost.** No date, duration, calendar sequence, effort figure or headcount appears below. Where ordering matters it is expressed as a dependency — a constraint cannot be declared before the data that violates it has been dealt with — and never as a schedule. The staging of the work belongs to [the recommended path](../migration/02-recommended-path.md). One class of date does appear throughout and is a different thing entirely: the `YYYYMMDD` values the estate stores in its own columns are data, not planning.
