...
This catalogue consolidates every LIFE400 clause term into two classes: elective **riders** (`option`), held in the in-memory `PM-RIDER-TABLE` and validated and priced by new-business underwriting [QCPYSRC/POLDATA.cpy:L88-L96] [QCBLLESRC/NBUWB.cbl:L345-L429], and embedded **contract terms plus base coverage** (`package`). Base death coverage is backed by a first-class copybook field, `PM-SUM-ASSURED` [QCPYSRC/POLDATA.cpy:L76]; the contestability, suicide, and expiry terms carry no rider code or copybook field and exist only as claim-adjudication logic [QCBLLESRC/CLMADJB.cbl:L206-L251]. Every term is classified `option` or `package`, and each description is drawn only from the COBOL message-string literals and comments cited on its row — no benefit behaviour is asserted beyond what the source states, and anything the source leaves unstated is marked `NOT SPECIFIED IN SOURCE`.
...
These terms carry no rider code; they are the always-present base coverage backing `PM-SUM-ASSURED` [QCPYSRC/POLDATA.cpy:L76] and the conditions/exclusions evaluated automatically during claim adjudication [QCBLLESRC/CLMADJB.cbl:L206-L251], so each is classified `package`. The source identifier is the paragraph rule tag (`CL-nnn`) shown in the Key Rule column, not a clause code (see [Observations](#observations)).
...
| — | Contestability | `package` | Condition under which a death inside the contestability period forces the claim into investigation rather than straight-through settlement — comment `DEATH WITHIN CONTESTABILITY PERIOD` [QCBLLESRC/CLMADJB.cbl:L208]; the window length is assigned as `MOVE 2` years for every plan by the claim plan-parameter loader [QCBLLESRC/CLMADJB.cbl:L145] [QCBLLESRC/CLMADJB.cbl:L149] [QCBLLESRC/CLMADJB.cbl:L153]. | Rule `CL-301`: `WS-DAYS-CONTESTABLE = PM-CONTESTABILITY-YRS * 365`; when `WS-DAYS-SINCE-ISSUE <= WS-DAYS-CONTESTABLE` the investigation status is set to pending (`'P'`) [QCBLLESRC/CLMADJB.cbl:L206-L213]. Per-plan value is 2 years (see [products.md](products.md)). | [QCBLLESRC/CLMADJB.cbl:L143-L155]; [QCBLLESRC/CLMADJB.cbl:L206-L213] |
| — | Suicide Exclusion | `package` | Exclusion that rejects a suicide occurring inside the suicide window — literal `SUICIDE WITHIN POLICY SUICIDE WINDOW: REJECTED` [QCBLLESRC/CLMADJB.cbl:L240]; the window length is assigned as `MOVE 2` years for every plan by the claim plan-parameter loader [QCBLLESRC/CLMADJB.cbl:L146] [QCBLLESRC/CLMADJB.cbl:L150] [QCBLLESRC/CLMADJB.cbl:L154]. | Rule `CL-401`: `WS-DAYS-SUICIDE-WINDOW = PM-SUICIDE-YRS * 365`; when `PM-CAUSE-SUICIDE` and the death falls inside the window, it sets code 21 and the claim decision to reject (`'R'`) [QCBLLESRC/CLMADJB.cbl:L232-L244]. | [QCBLLESRC/CLMADJB.cbl:L143-L155]; [QCBLLESRC/CLMADJB.cbl:L232-L244] |
...
All clauses attach to the single insured life on the base Term Life policy and apply uniformly across the three products T1001, T2001, and T6501: the claim plan-parameter loader assigns the same contestability, suicide, and grace windows to every plan [QCBLLESRC/CLMADJB.cbl:L143-L155], and rider validation applies the same code-based rules regardless of plan code [QCBLLESRC/NBUWB.cbl:L345-L383] (see [products.md](products.md)). The three elective riders each add an independent rider sub-exposure on top of the base death benefit (see [exposures.md](exposures.md)): only an active `'ADB01'` rider increases the death payout, and only on accidental death [QCBLLESRC/CLMADJB.cbl:L259-L270]. The `package` terms are not elective — base death coverage is always present, while contestability, the suicide exclusion, and expiry are evaluated automatically at claim time [QCBLLESRC/CLMADJB.cbl:L206-L251].
...
%% Source: QCPYSRC/POLDATA.cpy L88-L96; QCBLLESRC/NBUWB.cbl L345-L429; QCBLLESRC/CLMADJB.cbl L206-L251; 2-year contestability/suicide window labels assigned per plan in claim plan-loader QCBLLESRC/CLMADJB.cbl L143-L155
```
...
- **Embedded terms have no clause-level identity in source.** Contestability, the suicide exclusion, and expiry exist only as inline claim-adjudication logic tagged `CL-301`/`CL-401`/`CL-402`; no rider code, table row, or copybook field names them as clauses [QCBLLESRC/CLMADJB.cbl:L206-L251]. Recorded, not remediated.
