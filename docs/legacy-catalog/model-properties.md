...
The fields below carry an explicit default in source — either a definition-time `VALUE` clause or a run-time `MOVE`-based initialisation, distinguished as such in the Evidence column; every coded property not listed here has `Default` = `NOT SPECIFIED IN SOURCE`, because no level-88 coded domain outside this table carries a `VALUE` clause on its copybook field across the control-area, insured, benefit, servicing, and claim sections [QCPYSRC/POLDATA.cpy:L22-L34] [QCPYSRC/POLDATA.cpy:L60-L71] [QCPYSRC/POLDATA.cpy:L78-L96] [QCPYSRC/POLDATA.cpy:L118-L168]. This reference exists so that each populated `Default` cell above is traceable to its exact source of evidence and no default is fabricated.
...
None; all coded model properties discovered in source are catalogued in the tables above — the complete set of level-88 coded domains is defined within the single shared record `WS-POLICY-MASTER-REC` [QCPYSRC/POLDATA.cpy:L14-L175], which is the authoritative source inventory for every coded field in the system.
...
- **The NB-303 decline branch is literally coded but operationally unreachable in valid new business.** The `PM-UW-CLASS` derivation reproduces the literal NB-303 test — smoker **and** issue age > 60 **and** sum assured > 25000000000000 [QCBLLESRC/NBUWB.cbl:L288-L296] — in both the FEEL expression and the flowchart above, but a valid new-business application can never reach it: `1200-VALIDATE-APPLICATION` rejects any issue age above the plan maximum (60 across all three plans) at NB-202 [QCBLLESRC/NBUWB.cbl:L230-L237] and the caller returns on the non-zero result code before `1300-DETERMINE-UW-CLASS` runs [QCBLLESRC/NBUWB.cbl:L94-L101], while `PM-SUM-ASSURED PIC 9(13)V99` [QCPYSRC/POLDATA.cpy:L76] holds at most 13 integer digits and so can never exceed the 14-digit literal `25000000000000`. In practice a declined (`'DP'`) underwriting class is instead produced by the occupation-class-4 auto-decline at NB-206 [QCBLLESRC/NBUWB.cbl:L261-L268], which sets `'DP'` and exits before paragraph 1300. Recorded, not remediated.
