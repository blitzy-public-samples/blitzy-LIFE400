...
Four commands operate the set, and each reads one of the four configuration files at the repository root: `requirements-docs.txt`, `mkdocs.yml`, `.markdownlint-cli2.jsonc`, and `.mlc-config.json`. **Two runtimes are prerequisites, and each floor below is the one the pinned tool itself declares rather than a preference: Python 3.8 or newer**, because `mkdocs==1.6.1` and `mkdocs-material==9.7.7` both declare `Requires-Python >=3.8`; **and Node 22 or newer, with the `npx` that ships with npm**, because `markdownlint-cli2@0.23.2` declares `engines.node >=22` — the `@mermaid-js/mermaid-cli@11.16.0` used to parse the assessment's diagrams declares `^18.19 || >=20.0`, so Node 22 satisfies it as well, and `markdown-link-check@3.15.0` declares no engine constraint at all. `npx` runs each validator at its pinned version without installing anything, which is deliberate: **this repository carries no `package.json`**, because a Node manifest in an ILE COBOL, ILE CL, and DDS estate would misrepresent it to every reader and tool that inspects the tree. None of these prerequisites is a dependency of LIFE400 itself — they build and check the documentation, and the system's own runtime baseline is unchanged and stated at the end of this file.
