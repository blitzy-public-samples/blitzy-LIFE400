...
| `PM-CURRENCY-CODE` | `X(03)` | [`QCPYSRC/POLDATA.cpy:L35`]; `POLMST.CURCD` [`QDDSSRC/POLMST.pf:L29`] | Control / processing | Single-currency operational value carried per policy. In this mono-currency system there is no source-backed multi-currency domain, so it is a runtime value rather than a product-defining term. |
...
| COBOL/DDS field | PIC / DDS type | Source [file:line] | Category | Justification (why runtime, not product config) |
| --- | --- | --- | --- | --- |
| `PM-ATTAINED-AGE` | `9(03)` | [`QCPYSRC/POLDATA.cpy:L59`] | Derived / rating-adjustment | Current (attained) age computed at runtime from date-of-birth vs. the process date; a derived value, never configured on the product. (Contrast: `PM-ISSUE-AGE` is a mapped line field — see matrix §A.) |
...
The two `PM-RIDER-TABLE` [`QCPYSRC/POLDATA.cpy:L88-L96`] elements that are rating inputs/outputs. (Contrast: `PM-RIDER-CODE`, `PM-RIDER-SUM-ASSURED`, `PM-RIDER-STATUS` ARE mapped as coverage-clause discriminator / term / status — see `traceability-matrix.md` §B.)

| COBOL/DDS field | PIC / DDS type | Source [file:line] | Category | Justification (why runtime, not product config) |
| --- | --- | --- | --- | --- |
| `PM-RIDER-RATE` | `9(02)V9999` | [`QCPYSRC/POLDATA.cpy:L92`] | Rider rating / premium | Per-rider rating rate slot; a rider premium computation input, not product config. The applicable rider rates are hard-coded literals inside `1700-CALCULATE-RIDER-PREMIUM` [`QCBLLESRC/NBUWB.cbl:L405-L431`]. |
...
The `PM-PREMIUM-RESULTS` group [`QCPYSRC/POLDATA.cpy:L98-L106`] — all outputs of the COBOL premium-calculation paragraphs `1600-CALCULATE-BASE-PREMIUM` and `1700-CALCULATE-RIDER-PREMIUM` [`QCBLLESRC/NBUWB.cbl:L388-L431`] (and totals in `1800`). Every value here is computed per policy; none is a product definition.
...
| `PM-GROSS-ANNUAL-PREMIUM` | `9(13)V99` | [`QCPYSRC/POLDATA.cpy:L101`]; premium calc [`QCBLLESRC/NBUWB.cbl:L388-L431`] | Computed premium result | Gross premium (base + rider totals); computed result. |
| `PM-TAX-AMOUNT` | `9(13)V99` | [`QCPYSRC/POLDATA.cpy:L102`]; premium calc [`QCBLLESRC/NBUWB.cbl:L388-L431`] | Computed premium result | Computed tax amount (tax rate applied to premium). The tax *rate* is product config (mapped edition value); the computed *amount* is runtime. |
| `PM-TOTAL-ANNUAL-PREMIUM` | `9(13)V99` | [`QCPYSRC/POLDATA.cpy:L103`]; `POLMST.ANPREM 15S2` [`QDDSSRC/POLMST.pf:L59`] | Computed premium result | Computed total annual premium, persisted per policy; a stored result, not a product value. |
| `PM-MODAL-PREMIUM` | `9(13)V99` | [`QCPYSRC/POLDATA.cpy:L104`]; `POLMST.MODPREM 15S2` [`QDDSSRC/POLMST.pf:L61`] | Computed premium result | Computed modal (per-billing-period) premium, persisted per policy; computed result. |
| `PM-OUTSTANDING-PREMIUM` | `9(13)V99` | [`QCPYSRC/POLDATA.cpy:L105`]; `POLMST.OUTPREM 15S2` [`QDDSSRC/POLMST.pf:L63`] | Computed premium result | Outstanding premium balance maintained per policy; a runtime balance, not product config. |
| `PM-PREMIUM-DELTA` | `S9(13)V99` | [`QCPYSRC/POLDATA.cpy:L106`]; `SVCPF.PREMDLT 15S2` [`QDDSSRC/SVCPF.pf:L45`] | Computed premium result | Signed premium delta produced by a servicing amendment; a per-transaction computed result held in `SVCPF`. |
